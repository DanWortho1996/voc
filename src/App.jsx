...
// Correct environment variable usage (Vite or Create React App)
...
const App = () => {
  ...
  useEffect(() => {
    ...
    socket.on("nextPlayer", ({ nextPlayer, room: gameRoom }) => {
      console.log("nextPlayer event received:", { nextPlayer, gameRoom });
      if (nextPlayer === playerName && gameRoom === room) {
        console.log("🎮 Starting game for:", nextPlayer);
        setGameStarted(true);
      }
    });
  ...
  useEffect(() => {
    socket.on("playersInRoom", (players) => {
      console.log("Players in room updated:", players);
